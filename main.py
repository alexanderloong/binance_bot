# Binance Trading Bot - v2.1.0 (Production Stable Release 2026)
import time
...
def main():
    ...
    while True:
        try:
            ...
            should_sleep = (
                pre_close_buffer < seconds_remaining < (tf_seconds - post_open_buffer)
            )

            if should_sleep:
                sleep_duration = seconds_remaining - pre_close_buffer
                logger.info(
                    f"Waiting {int(sleep_duration)}s until next candle close area..."
                )
            ...
            notifier.send_lark_message(
                "🛑 **Binance Bot Stopped (User / KeyboardInterrupt)**"
            )
            break
        except Exception as e:
            ...
            logger.critical(
                f"CRITICAL: Bot process is crashing: {type(be).__name__}: {be}"
            )
            notifier.send_lark_message(
