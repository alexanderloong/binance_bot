...
def plot_performance(df=None, trades=None, res=None):
    ...
    fig, (ax1, ax2, ax3) = plt.subplots(3, 1, figsize=(14, 12), sharex=True, gridspec_kw={'height_ratios': [2, 1.5, 0.8]})
    
    # Plot 1: Price Chart
    ax1.plot(df_st['timestamp'], df_st['close'], color='#1f77b4', linewidth=1, label='BTC/USDT Price')
    ...
    # Plot 2: Equity Curve
    ax2.plot(equity_df['time'], equity_df['balance'], color='#00ff88', linewidth=1.5, label='Equity (USDT)')
    ax2.axhline(y=1000, color='gray', linestyle='--', alpha=0.5, label='Initial Balance')
    ...
    ax2.annotate(f'Peak: ${max_bal:.2f}', xy=(max_date, max_bal), xytext=(10, 10), 
                 textcoords='offset points', color='white', fontweight='bold',
                 bbox=dict(boxstyle="round,pad=0.3", fc="#00aa55", ec="none", alpha=0.8))

    ax2.set_title('Strategy Performance: Equity Curve', fontsize=14, fontweight='bold', pad=15)
    ax2.set_ylabel('Balance (USDT)', fontsize=12)
    ax2.legend(loc='upper left')
    
    # Color area under curve
    ax2.fill_between(equity_df['time'], equity_df['balance'], 1000, where=(equity_df['balance'] >= 1000), 
    ...
    ax3.fill_between(equity_df['time'], equity_df['drawdown'], 0, color='#ff4444', alpha=0.2)
    ...
    plt.figtext(0.15, 0.12, stats_text, fontsize=10, bbox=dict(facecolor='white', alpha=0.8, boxstyle='round'))
