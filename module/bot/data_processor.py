...
class DataProcessor:
    ...
    def prepare_all_indicators(df: pd.DataFrame) -> pd.DataFrame:
        ...
        df_final[f"EMA_{settings.EMA_LENGTH}"] = DataProcessor.calculate_ema(
            df_final, length=settings.EMA_LENGTH
        )[f"EMA_{settings.EMA_LENGTH}"]
        ...
        df_final = DataProcessor.calculate_volume_ma(
            df_final, length=settings.VOLUME_MA_LENGTH
        )

        ema_slope_length = getattr(
            settings, "EMA_SLOPE_EMA_LENGTH", settings.EMA_LENGTH
        )
        df_final[f"EMA_{ema_slope_length}"] = DataProcessor.calculate_ema(
            df_final, length=ema_slope_length
        )[f"EMA_{ema_slope_length}"]
    ...
    def calculate_heikin_ashi(df: pd.DataFrame) -> pd.DataFrame:
        ...
        heikin_ashi_df["ha_high"] = heikin_ashi_df[["high", "ha_open", "ha_close"]].max(
            axis=1
        )
        heikin_ashi_df["ha_low"] = heikin_ashi_df[["low", "ha_open", "ha_close"]].min(
            axis=1
        )
    ...
    def resample_to_htf(df: pd.DataFrame, htf: str) -> pd.DataFrame:
        ...
        # FIX: removed redundant first resample — only keep the full OHLCV agg below
        df_htf = (
            df_indexed.resample(offset)
            .agg(
                open=("open", "first"),
                high=("high", "max"),
                low=("low", "min"),
    ...
    def calculate_supertrend(
        df: pd.DataFrame,
        length: int = settings.SUPERTREND_LENGTH,
        multiplier: float = settings.SUPERTREND_FACTOR,
    ) -> pd.DataFrame:
        """
        ...
        for i in range(1, len(df)):
            ...
            if (
                basic_upperband.iloc[i] < upperband.iloc[i - 1]
                or close[i - 1] > upperband.iloc[i - 1]
            ):
            ...
            if (
                basic_lowerband.iloc[i] > lowerband.iloc[i - 1]
                or close[i - 1] < lowerband.iloc[i - 1]
            ):
    ...
    def check_bearish_divergence(
        df: pd.DataFrame, lookback: int = 10, min_rsi: int = 60
    ) -> bool:
        """
