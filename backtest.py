...
from config import (
    ...
    RSI_LONG_THRESHOLD,
    VOLUME_MA_LENGTH,
    EMA_SLOPE_EMA_LENGTH,
    EMA_SLOPE_LOOKBACK,
    EMA_SLOPE_THRESHOLD,
    REDUCED_POSITION_SIZE_PERCENT,
    RSI_DIV_LOOKBACK,
    RSI_DIV_MIN_RSI,
    RSI_DIV_PARTIAL_CLOSE_PCT,
    settings,
    TAKER_FEE_RATE,
)
...
def simulate(
    ...
    reduced_size_percent=REDUCED_POSITION_SIZE_PERCENT,
    slope_ema_length=EMA_SLOPE_EMA_LENGTH,
    slope_lookback=EMA_SLOPE_LOOKBACK,
    use_divergence_filter=True,
    div_lookback=RSI_DIV_LOOKBACK,
    div_min_rsi=RSI_DIV_MIN_RSI,
    div_partial_pct=RSI_DIV_PARTIAL_CLOSE_PCT,
    roi_tp=settings.ROI_TP,
):
    ...
    for i in range(1, len(df)):
        ...
        if use_divergence_filter and i > div_lookback + 5:
            ...
            if len(peak_indices) >= 2:
                ...
                if curr_idx - p2_idx <= 3:
                    if (
                        arr_high[p2_idx] > arr_high[p1_idx]
                        and arr_rsi[p2_idx] < arr_rsi[p1_idx]
                    ):
                        bearish_div = True
        ...
        if roi_tp > 0 and position_amt != 0:
            ...
            if current_roi >= (roi_tp / 100.0):  # roi_tp in percent
                raw_pnl = (
                    (price - entry_price) * position_amt
                    if position_amt > 0
                    else (entry_price - price) * abs(position_amt)
                )
                fee = (price * abs(position_amt)) * commission_rate
                pnl = raw_pnl - fee
                balance += pnl
        ...
        rsi_long_ok = (
            (rsi_val < rsi_overbought and rsi_val > rsi_long_threshold)
            if use_rsi_filter
            else True
        )
...
def get_backtest_data(limit=LIMIT):
    ...
    cache_file = os.path.join(
        cache_dir, f"backtest_data_{symbol_clean}_{TIMEFRAME}.csv"
    )
        ...
        if file_age < tf_seconds:
            ...
            try:
                ...
                if not df.empty:
                    ...
                    if len(df) < limit:
                        print(
                            f"⚠️ Cache has {len(df)} candles, but {limit} requested. Fetching fresh data."
                        )
                        should_fetch = True
                    else:
                        should_fetch = False
            ...
            print(
                f"🔄 Cache is stale (Age: {int(file_age)}s >= {tf_seconds}s threshold). Fetching fresh data..."
            )

    if should_fetch:
        print(
            f"Fetching {limit} historical candles from LIVE Binance (multi-threaded)..."
        )
        ...
        try:
            ...
            print(
                f"Plan: Fetch {num_batches} batches (limit=1500) using {WORKERS} threads."
            )
            ...
            print(
                f"Estimated Weight: {int(estimated_weight)} / {safe_limit} (Max 2400)"
            )
            ...
            print(
                f"Estimated time: ~{int(num_batches/(WORKERS/SLEEP))} seconds (Safety Mode needed for large data)"
            )
            ...
            if len(end_times) < num_batches:
                print(
                    f"ℹ️  Trimmed to {len(end_times)} batches (history limit reached before 2019-10-01)."
                )
            ...
            def fetch_single_batch(end_ts):
                ...
                while retries > 0:
                    try:
                        return live_client.klines(
                            symbol_clean, interval=TIMEFRAME, limit=1500, endTime=end_ts
                        )
                    except Exception as e:
                        err_msg = str(e)
                        retry_after = 5
                        ...
                        if "418" in err_msg or "429" in err_msg:
                            if "retry-after" in err_msg.lower():
                                try:
                                    retry_after = int(
                                        err_msg.split("'retry-after': '")[1].split("'")[
                                            0
                                        ]
                                    )
                                except:
...
def run_backtest():
    ...
    print(
        f"Strategy: EMA {EMA_LENGTH}, SuperTrend {SUPERTREND_LENGTH}/{SUPERTREND_FACTOR}, ADX > {ADX_THRESHOLD}, SL: {ATR_MULTIPLIER}xATR, ROI TP: {settings.ROI_TP}%"
    )
    ...
    df_st[f"EMA_{EMA_SLOPE_EMA_LENGTH}"] = DataProcessor.calculate_ema(
        df_st, length=EMA_SLOPE_EMA_LENGTH
