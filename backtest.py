...
def simulate(
    ...
    for i in range(1, len(df)):
        ...
        if position_amt > 0 and curr_trend == -1:
            ...
            balance += pnl
            trades.append(
                {"time": execution_time, "type": "CLOSE_LONG", "price": price, "pnl": pnl}
            )
            position_amt = 0
            ...
            balance += pnl
            trades.append(
                {"time": execution_time, "type": "CLOSE_SHORT", "price": price, "pnl": pnl}
            )
            position_amt = 0
        ...
        if position_amt > 0 and price <= stop_loss_price:
            ...
            balance += pnl
            trades.append(
                {"time": execution_time, "type": "STOP_LOSS_LONG", "price": stop_loss_price, "pnl": pnl}
            )
            position_amt = 0
            ...
            balance += pnl
            trades.append(
                {"time": execution_time, "type": "STOP_LOSS_SHORT", "price": stop_loss_price, "pnl": pnl}
            )
            position_amt = 0
    ...
    for t in trades:
        if "OPEN" in t["type"]:
            ...
            is_open = True
        elif any(x in t["type"] for x in ["CLOSE", "FINAL_CLOSE", "LIQUIDATION"]):
...
def run_backtest():
    ...
    log(f"Annualized PnL: {res['annualized_pnl_pct']:.2f}%")
    ...
    log(f"Calmar Ratio: {res['calmar_ratio']:.2f}")
