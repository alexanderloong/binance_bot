...
def run_optimization():
    ...
    periods = range(7, 21, 1)               
    multipliers = [m / 10.0 for m in range(15, 55, 5)]  
    
    args_list = list(itertools.product(periods, multipliers))
    total_iterations = len(args_list)
    print(f"Starting search across {total_iterations} combinations using {mp.cpu_count()} processes...\n")
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=mp.cpu_count()) as executor:
        ...
        future_to_args = {executor.submit(evaluate_params, (p, m, df)): (p, m) for p, m in args_list}
        for future in concurrent.futures.as_completed(future_to_args):
            ...
            print(f"\rProgress: {completed}/{total_iterations} | Tested P={p}, M={m}...", end='', flush=True)
    ...
    if best_params:
        ...
        best_result = next((r for r in results if r['period'] == best_params[0] and r['multiplier'] == best_params[1]), None)
        if best_result:
